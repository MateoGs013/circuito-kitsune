...
@section('title', 'Eliminar Transmisión · Circuito Kitsune')
@section('section_label', 'PANEL ADMIN · ELIMINAR BLOG')
...
@section('content')
    <section class="admin-card" style="max-width: 600px; border-color: var(--color-ember);">
        ...
        <h1 class="t-display-sm" style="color: var(--color-bone); margin-bottom: 1.5rem;">¿ELIMINAR TRANSMISIÓN?</h1>
        <p style="font-family: var(--font-mono); color: var(--color-bone-dim); margin-bottom: 2.5rem; font-size: var(--text-mono-sm);">
            Estás por eliminar permanentemente la transmisión <strong>"{{ $post->title }}"</strong>. Esta acción destruirá la entrada del archivo y purgará los registros asociados en el circuito.
        </p>

        <form action="{{ route('admin.posts.destroy', $post->id) }}" method="POST" style="display: flex; gap: 1.5rem; flex-wrap: wrap;">
            @csrf
            @method('DELETE')
            ...
            <a href="{{ route('admin.posts.index') }}" class="bracket-cta bracket-cta--bone" style="flex: 1; min-width: 200px; text-decoration: none; justify-content: space-between; text-align: center;">
                <span aria-hidden="true">[</span>

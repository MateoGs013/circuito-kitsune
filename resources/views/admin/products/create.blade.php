...
@section('title', 'Nueva Máscara · Circuito Kitsune')
@section('section_label', 'PANEL ADMIN · NUEVA MÁSCARA')
...
@section('content')
    <section class="admin-card" style="max-width: 800px;">
        ...
        <x-system-tag label="CENTRAL · REGISTRO DE MÁSCARA" pulse />
        <h1 class="t-display-sm" style="color: var(--color-bone); margin-bottom: 2.5rem;">CREAR MÁSCARA.</h1>

        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" novalidate style="display: flex; flex-direction: column; gap: 2rem;">
            @csrf
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                <div>
                    <label for="name" class="admin-label">[ NOMBRE (ej. MÁSCARA ONI: ECLIPSE ROJO) ]</label>
                ...
                <div>
                    <label for="code" class="admin-label">[ CÓDIGO (ej. CK-ONI-09) ]</label>
                    <input type="text" name="code" id="code" value="{{ old('code') }}" class="admin-input @error('code') is-invalid @enderror" @error('code') aria-invalid="true" aria-errormessage="error-code" @enderror>
                    @error('code')
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
                <div>
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category', 'Máscara Oni') }}" class="admin-input @error('category') is-invalid @enderror" @error('category') aria-invalid="true" aria-errormessage="error-category" @enderror>
                    @error('category')
                ...
                <div>
                    ...
                    <input type="text" name="rarity" id="rarity" value="{{ old('rarity', 'Rara de señal') }}" class="admin-input @error('rarity') is-invalid @enderror" @error('rarity') aria-invalid="true" aria-errormessage="error-rarity" @enderror>
                    @error('rarity')
                ...
                <div>
                    ...
                    <input type="text" name="district" id="district" value="{{ old('district', 'NEO-SHIBUYA') }}" class="admin-input @error('district') is-invalid @enderror" @error('district') aria-invalid="true" aria-errormessage="error-district" @enderror>
                    @error('district')
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', 15000) }}" class="admin-input @error('price') is-invalid @enderror" @error('price') aria-invalid="true" aria-errormessage="error-price" @enderror>
                    @error('price')
                ...
                <div>
                    ...
                    <select name="status" id="status" class="admin-input @error('status') is-invalid @enderror" @error('status') aria-invalid="true" aria-errormessage="error-status" @enderror>
                        <option value="disponible" @if(old('status') === 'disponible') selected @endif>Disponible</option>
                        <option value="próxima" @if(old('status') === 'próxima') selected @endif>Próxima</option>
                        <option value="agotada" @if(old('status') === 'agotada') selected @endif>Agotada</option>
                    </select>
                    @error('status')
                ...
                <div>
                    ...
                    <select name="dominant_color" id="dominant_color" class="admin-input @error('dominant_color') is-invalid @enderror" @error('dominant_color') aria-invalid="true" aria-errormessage="error-dominant-color" @enderror>
                        <option value="cyan" @if(old('dominant_color') === 'cyan') selected @endif>Cyan</option>
                        <option value="red" @if(old('dominant_color') === 'red') selected @endif>Red</option>
                        <option value="violet" @if(old('dominant_color') === 'violet') selected @endif>Violet</option>
                        <option value="gold" @if(old('dominant_color') === 'gold') selected @endif>Gold</option>
                        <option value="magenta" @if(old('dominant_color') === 'magenta') selected @endif>Magenta</option>
                        <option value="blue" @if(old('dominant_color') === 'blue') selected @endif>Blue</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="short_description" id="short_description" rows="2" class="admin-input @error('short_description') is-invalid @enderror" @error('short_description') aria-invalid="true" aria-errormessage="error-short-desc" @enderror>{{ old('short_description') }}</textarea>
            ...
            <div>
                ...
                <textarea name="long_description" id="long_description" rows="6" class="admin-input @error('long_description') is-invalid @enderror" @error('long_description') aria-invalid="true" aria-errormessage="error-long-desc" @enderror>{{ old('long_description') }}</textarea>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1.5rem;">
                <div>
                    ...
                    <input type="number" name="signal_level" id="signal_level" value="{{ old('signal_level', 85) }}" min="0" max="99" class="admin-input @error('signal_level') is-invalid @enderror" @error('signal_level') aria-invalid="true" aria-errormessage="error-signal" @enderror>
                    @error('signal_level')
                ...
                <div>
                    ...
                    <input type="number" name="agility" id="agility" value="{{ old('agility', 70) }}" min="0" max="99" class="admin-input @error('agility') is-invalid @enderror" @error('agility') aria-invalid="true" aria-errormessage="error-agility" @enderror>
                    @error('agility')
                ...
                <div>
                    ...
                    <input type="number" name="spirit" id="spirit" value="{{ old('spirit', 90) }}" min="0" max="99" class="admin-input @error('spirit') is-invalid @enderror" @error('spirit') aria-invalid="true" aria-errormessage="error-spirit" @enderror>
                    @error('spirit')
                ...
                <div>
                    ...
                    <input type="number" name="ferocity" id="ferocity" value="{{ old('ferocity', 80) }}" min="0" max="99" class="admin-input @error('ferocity') is-invalid @enderror" @error('ferocity') aria-invalid="true" aria-errormessage="error-ferocity" @enderror>
                    @error('ferocity')
            ...
            <div>
                <label for="image" class="admin-label">[ IMAGEN ADJUNTA / ARCHIVO ]</label>
            ...
            <button type="submit" class="bracket-cta bracket-cta--cyan" style="background: none; border: none; cursor: pointer; width: 100%; justify-content: space-between; margin-top: 1rem;">
                ...
                <span class="bracket-cta__text">&gt;_ REGISTRAR MÁSCARA (GUARDAR)</span>
                <span aria-hidden="true">]</span>

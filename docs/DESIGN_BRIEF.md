...
| 2026-05-05 | sesión autopilot — implementación visual completa | 5 vistas implementadas + desviaciones documentadas en `docs/PROGRESS.md`. Cambios principales: (1) **R13 / R17 / R21** — familia mono cambió de `VT323` a `IBM Plex Mono` (weights 400-700); cliente: "todo lo que es tipografía mono casi no se ve". (2) **§ 8.1.1** — scan-grid full del hero reemplazado por divisores 1px ink al 28% entre las 3 columnas; cliente: "la grilla está siendo molesta". (3) **§ 8.1.1** — ambient letters scattered eliminadas (catalogadas en § 14.5 post-MVP). (4) **§ 8.1.1** — segundo CTA `LEER TRANSMISIONES` adaptado a link editorial con border-bottom (en lugar de bracket-cta) para preservar jerarquía 1-CTA dominante del lenguaje Utopia. (5) **R59** — dot de tagline = ink puro sobre block-ember (R59 pedía ember, invisible sobre rojo); coherente con § 5.1.4 inversión. (6) **R40** — header fijo bg ink + border-bottom ash **se mantiene** (cliente rechazó propuesta de header transparente). Acceptance § 12 verificado completo. Bundles: CSS 10.48 KB gzip / JS app+lenis 38.6 KB gzip. |
